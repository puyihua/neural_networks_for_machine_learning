...
count = 0;
tiny = exp(-30);  % pyh :prevent the log function from overflow??
log_train_CE = zeros(1,epochs);
log_valid_CE = zeros(1,epochs);
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    % BACK PROPAGATE.
    %% OUTPUT LAYER.
    hid_to_output_weights_gradient =  hidden_layer_state * error_deriv';  %pyh: error_deriv = d(cost)/dy * dy/dz
    ...
    % FILL IN CODE. Replace the line below by one of the options.
    % replaced :back_propagated_deriv_2 = zeros(numhid2, batchsize);
    % Options
...
log_train_CE(epoch) = trainset_CE;
log_valid_CE(epoch) = CE;
end
